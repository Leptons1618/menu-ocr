...
\subsubsection{Greedy Matching (Baseline)}
...
\subsubsection{Bipartite Matching (Improved)}
...
\begin{equation}
C_{ij} = w_v \cdot \frac{|y_i - y_j|}{\bar{h}} + w_h \cdot \max(0, x_i - x_j) + w_c \cdot \mathbbm{1}[\text{invalid\_column}]
\end{equation}
...
\begin{equation}
\pi^* = \arg\min_{\pi} \sum_{i=1}^{n} C_{i, \pi(i)}
\end{equation}

This global optimization prevents conflicts where multiple items claim the same price.

\subsection{Column-Aware Layout Segmentation}

Multi-column menus require explicit column detection. We cluster text elements by x-coordinate using DBSCAN:

\begin{equation}
\text{Columns} = \text{DBSCAN}(\{x_{\text{center}}^{(i)}\}_{i=1}^{N}, \epsilon, \text{min\_pts})
\end{equation}

The price column is identified by:
\begin{itemize}
    \item Rightmost position
    \item High digit ratio in contained text
    \item Narrow width relative to content columns
\end{itemize}

\subsection{Hierarchy Detection}

We enforce valid label sequences using a finite-state machine with Viterbi decoding:

\begin{equation}
\hat{y}_{1:T} = \arg\max_{y_{1:T}} \prod_{t=1}^{T} P(y_t | x_t) \cdot P(y_t | y_{t-1})
\end{equation}

Valid transitions include:
\begin{itemize}
    \item SECTION\_HEADER $\rightarrow$ GROUP\_HEADER, ITEM\_NAME
    \item GROUP\_HEADER $\rightarrow$ ITEM\_NAME, GROUP\_HEADER
    \item ITEM\_NAME $\rightarrow$ ITEM\_PRICE, ITEM\_DESCRIPTION, ITEM\_NAME
\end{itemize}
...
\textbf{Recommendation}: Avoid training on receipt datasets (CORD-v2, SROIE) for menu extraction unless domain adaptation is performed. ML models should serve as secondary signals, not primary decision makers.

\subsection{Per-Stage Error Decomposition}
...
\midrule
OCR Detection & $\sim$15\% & 15\% & Partial \\
OCR Recognition & $\sim$8\% & 22\% & No \\
Classification & $\sim$20\% & 38\% & Yes \\
Price Matching & $\sim$25\% & 54\% & Yes \\
\bottomrule
\end{tabular}
\caption{Error contribution by pipeline stage (estimated from test set)}
...
\begin{enumerate}
    ...
    \item \textbf{Use Bipartite Matching} for price-item association to avoid greedy assignment conflicts
    \item \textbf{Detect Columns Explicitly} before attempting reading order resolution
\end{enumerate}
...
\begin{itemize}
    \item \textbf{Limited test set}: Evaluation on only 4 menu images (75 items) may not capture full layout diversity
    \item \textbf{English-only}: Results may not generalize to other languages or scripts
    \item \textbf{Single annotator}: Ground truth annotations by single annotator; no inter-annotator agreement measured
    \item \textbf{OCR coupling}: OCR errors not independently evaluated from classification errors
    \item \textbf{No confidence intervals}: Metrics reported without statistical uncertainty bounds
\end{itemize}
...
\begin{itemize}
    ...
    \item Price formats limited to common Western conventions
\end{itemize}

\subsection{Future Directions}

\begin{enumerate}
    \item \textbf{Menu-Specific Dataset}: Curate labeled menu dataset of 50+ images for robust evaluation
    \item \textbf{Vision-Language Models}: Fine-tune LayoutLMv3 or similar models on menu data
    \item \textbf{Detection-Recognition Split}: Decouple OCR stages for independent error analysis
    ...
    \item \textbf{Confidence Calibration}: Provide calibrated confidence scores for downstream filtering
\end{enumerate}

\section{Conclusion}

I presented a modular pipeline for structured menu extraction combining OCR with hybrid classification approaches. The key finding is that \textbf{domain mismatch} between receipt-trained models and menu images is the primary accuracy bottleneck---ML models achieving 91\% accuracy on CORD-v2 drop to $<$30\% F1 on menus.

\subsection{Updated Results (v2.1)}
...
\textbf{v2.1 Final} & \textbf{75.8\%} & \textbf{70.1\%} & \textbf{72.8\%} & 34.8\% & 20 \\
\bottomrule
\end{tabular}
\caption{Improvement trajectory across versions (491 total items)}
...
\begin{enumerate}
    \item \textbf{Empirical demonstration of domain mismatch} between receipt-trained models and menu extraction, with quantitative analysis
    \item \textbf{Design principles for faithful extraction} that avoid hallucination through rule-based constraints
    \item \textbf{Modular pipeline architecture} enabling component-level ablation and replacement
    \item \textbf{Global bipartite matching} for price-item association replacing greedy heuristics
    \item \textbf{Open evaluation framework} for menu extraction benchmarking
\end{enumerate}
