...
\begin{abstract}
We present a modular pipeline for extracting structured data from restaurant menu images. Our system combines optical character recognition (OCR) with multiple classification approaches—rule-based heuristics, traditional machine learning models, and ensemble methods—to produce schema-compliant JSON output without hallucinating structure not present in the source image. We provide a comprehensive comparison of OCR backends and evaluate six classification approaches on a hand-labeled test set. Our experiments reveal that domain-specific rule-based classification outperforms general-purpose machine learning models trained on receipt datasets, achieving 35.6\% F1 score with 49\% price extraction accuracy. We analyze the causes of this performance gap and propose guidelines for production deployment.
\end{abstract}
...
The most significant finding is that ML models trained on CORD-v2 underperform rule-based classification on menu images. We attribute this to fundamental domain differences:
...
Based on our experiments, we provide the following guidelines for production deployment:
